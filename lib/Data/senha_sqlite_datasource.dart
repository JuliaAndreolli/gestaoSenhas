...
class senhaSQLiteDatasource {
  ...
  Future<void> atualizarSenha(SenhaEntity senha) async {
    ...
    await db.transaction((txn) async {
      await txn.rawUpdate(
          'update $SENHA_TABLE_NAME set $SENHA_COLUMN_DESCRICAO = ?, $SENHA_COLUMN_EMAIL = ?, $SENHA_COLUMN_SENHA = ? where id = ?',
          [senha.descricao, senha.email, senha.senha]);
    });
  }
  ...
  Future<bool> getSenhaEmail(String filtro) async {
    final Database db = await DatabaseConnection.getDatabase();
    List<Map> dbResult = await db.rawQuery(
        'SELECT * from $SENHA_TABLE_NAME where $SENHA_COLUMN_EMAIL = ?',
        ['$filtro']);
